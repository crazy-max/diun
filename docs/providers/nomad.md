...
Nomad namespaces to query from. If neither `namespaces` nor the deprecated `namespace` option is set, Diun queries
all namespaces accessible by the configured token.
...
!!! abstract "Environment variables"
    * `DIUN_PROVIDERS_NOMAD_NAMESPACES`
...
!!! warning
    This option is deprecated. Use [`namespaces`](#namespaces) instead.
